...
Thanks for using `fcc-broadband-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.

- [Report a bug](https://github.com/cyanheads/fcc-broadband-mcp-server/issues/new?template=bug_report.yml)
- [Request a feature](https://github.com/cyanheads/fcc-broadband-mcp-server/issues/new?template=feature_request.yml)

Both forms are structured, and filling in the fields is what makes an issue actionable.

PRs are welcome for typo-scale fixes. For anything larger, open an issue first — code changes go through my own workflows, so a precise issue with a reproduction is the fastest route to a fix.

## Server bug or framework bug?

`fcc-broadband-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:

- **This repo** — a tool returns wrong data, an FCC API call fails, a schema doesn't match reality, a description misleads the model.
...
3. **Redact anything sensitive.** Issues are public and permanent — no API keys, tokens, auth headers, internal URLs, or PII in code, logs, or stack traces. BDC credentials (`FCC_BDC_USERNAME`, `FCC_BDC_HASH_VALUE`) in particular.
